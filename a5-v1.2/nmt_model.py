...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        # print("in nmt_model: ", self.vocab.src.char2id)
        # Add `source_padded_chars` for character level padded encodings for source
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device = self.device) # size (sentence_length, batch_size, max_word_length)
        # print("sen, slen1, slen2: ", len(source), len(source[0]), len(source[1]))
        # print(source)
        # print(source_padded_chars)
        # (sen_len, b, word_len) = source_padded_chars.size()
        # print("sen_len, b, word_len: ", sen_len, b, word_len)

        # Add `target_padded_chars` for character level padded encodings for target
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device)
        # target_padded_chars = self.model_embeddings_source.forward(target_tensor_char)
