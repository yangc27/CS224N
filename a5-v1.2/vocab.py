...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sen in sents:
            sen_in_ids = []
            for word in sen:
                # word to char id list
                word_in_ids = [self.char2id[char] for char in word]
                word_in_ids.insert(0, self.start_of_word)
                word_in_ids.append(self.end_of_word)
                # append a word (char id list) to the sentence
                sen_in_ids.append(word_in_ids)
            word_ids.append(sen_in_ids)
        return word_ids
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        # print(word_ids)
        padded_sentences = pad_sents_char(word_ids, self.char2id['<pad>'])
        sents_var = torch.tensor(padded_sentences, dtype = torch.long, device = device)
