...
def pad_sents_char(sents, char_pad_token):
    ...
    sents_padded = []
    for sen in sents:
        sen_padded = [[char_pad_token] * max_word_length] * max_sent_len
        word_padded_list = []
        for word in sen:
            word_padded = [char_pad_token] * max_word_length
            word_padded[: min(len(word), max_word_length)] = word[: min(len(word), max_word_length)]
            word_padded_list.append(word_padded)
        
        sen_padded[: len(word_padded_list)] = word_padded_list[:]
        sents_padded.append(sen_padded)
