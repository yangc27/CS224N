...
class ParserModel(nn.Module):
    ...
    def forward(self, t):
        """ Run the model forward.
            ...
            PyTorch Notes:
                ...
                - When you apply your nn.Module to an input tensor `t` this function is applied to the tensor.
                    ...
                    the `forward` function would be called on `t` and the result would be stored in the `output` variable:
                        model = ParserModel()
        ...
        # Apply embed_to_hidden linear layer on embeddings
        hidden_output = self.embed_to_hidden(embed)

        # Apply relu on hidden output
        relu_output = nn.functional.relu(hidden_output)

        # Apply dropout layer to relu output
        dropout_output = self.dropout(relu_output)

        # Apply hidden_to_logits layer to dropout output
        logits = self.hidden_to_logits(dropout_output)
