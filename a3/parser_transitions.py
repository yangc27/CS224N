...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            raise ValueError("transition has to be S, LA, or RA")
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        # predict and perform transitions on minibatch
        while len(minibatch) > 0:
            transitions = model.predict(minibatch) # each transition corresponds to a PartialParse in minibatch
    ...
    for idx in range(len(sentences)):
        dependencies.append(partial_parses[idx].dependencies)  
    ### END YOUR CODE
